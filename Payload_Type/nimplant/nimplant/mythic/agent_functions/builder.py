...
class NimPlant(PayloadType):
    ...
    c2_profiles = ["http"]
    agent_path = pathlib.Path(".") / "nimplant" / "mythic"
    agent_code_path = pathlib.Path(".") / "nimplant" / "agent_code"
    agent_icon_path = agent_path / "agent_functions" / "nimplant.svg"
    ...
    async def build(self) -> BuildResponse:
        ...
        try:
            ...
            for c2 in self.c2info:
                ...
                for key, val in c2.get_parameters_dict().items():
                    ...
                    if key == 'AESPSK':
                        ...
                        aespsk_val = val["enc_key"] if val["enc_key"] is not None else ""
            ...
            # Removed optimization flag which increases size slightly. Upstream puppy is broken otherwise.
            ...
            command = f"nim {'c' if self.get_parameter('lang') == 'C' else 'cpp'} {'--os:linux --passL:-W --passL:-ldl' if self.get_parameter('os') == 'linux' else ''} -f --d:mingw {'--d:debug --hints:on --nimcache:' + agent_build_path.name if self.get_parameter('build') == 'debug' else '--d:release --hints:off'} {'--d:AESPSK=' + aespsk_val  if len(aespsk_val) > 2 else ''} --opt:size --passL:-flto --passL:-s {'--app:lib' if self.get_parameter('format') == 'dll' else ''} {'--embedsrc:on' if self.get_parameter('build') == 'debug' else ''} --cpu:{'amd64' if self.get_parameter('arch') == 'x64' else 'i386'} --out:{self.name}{out_ext} c2/base.nim"
